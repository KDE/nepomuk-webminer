    ...
    , m_url( info.absoluteFilePath() )
{
    ...
    connect(m_processTimer, SIGNAL(timeout()), this, SLOT(slotProcessTimerTimeout()));
}
...
void WebMinerIndexingJob::slotIndexedFile(int exitCode)
{
    ...
    if(exitCode == 1 && cfg.group( "General" ).readEntry( "debug mode", false )) {
        QFile errorLogFile(KStandardDirs::locateLocal("data", QLatin1String("nepomuk/webminer-indexer-error-log"), true));
